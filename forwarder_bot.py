...
async def show_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not task:
        await update.callback_query.edit_message_text("Error: Task not found."); return MAIN_MENU
...
async def get_batch_destination(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    count, errors = 0, 0
    try:
        # --- FIX #2: Use reliable copy method in batch and handle deleted messages ---
        ...
        for i, message in enumerate(messages_to_copy):
            if message: # Check if message exists (wasn't deleted)
                try:
                    await client.send_message(dest_id, file=message.media, message=message.text, link_preview=False)
                    count += 1
                except Exception:
                    errors += 1
            # We don't count deleted messages as errors
            
            if (i + 1) % 10 == 0: await status_msg.edit_text(f"Progress: {i+1}/{total} messages checked...")
            await asyncio.sleep(1.5) # Prevent flood waits
    except Exception as e:
...
async def main():
    global MY_ID
    # --- FIX #1: Increase timeouts for handling large media files ---
    application = (
        Application.builder()
        .token(BOT_TOKEN)
        .connect_timeout(30)
        .read_timeout(30)
        .write_timeout(60) # Increased to 60 seconds for uploads
        .build()
    )
